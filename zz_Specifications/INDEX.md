...
| [done/SPEC_MCP_TIME_ENTRY_TOOLS.md](done/SPEC_MCP_TIME_ENTRY_TOOLS.md) | Delegated actuals (`log_time_entry`): decision record + guarded design (HS 11634, 11710) | Done (v1.0.18, stage-verified 2026-08-06; ITM.Web/API prerequisite deployed to prod 2026-08-06, MCP prod deploy pending) |
| [done/SPEC_MCP_PROJECT_PROGRESS_TOOLS.md](done/SPEC_MCP_PROJECT_PROGRESS_TOOLS.md) | Project-level Seguimiento: `create_project_progress`, `update_project_progress`, entries in `get_project_progress` (HS 11761) | Done (v1.0.18, stage-verified 2026-08-06; ITM.Web/API prerequisite deployed to prod 2026-08-06, MCP prod deploy pending) |
...
| 2026-08-06 | OAuth (mcp_) session tokens were rejected by ALL v1 endpoints: classic validation compared the presented token only against the user's first tblUserToken row, and its expiry refresh stamped every row (the session-killer behind HS 11440). Fixed in ITM.Web (`clsCommon.ValidateToken` row-scoped, ticket `ITM.Web/zz_Tickets/2026-08-06-v1-token-validation-multi-row.md`), plus the `GetIsTaskEditiableByDateAndTask` sproc join bug. Both clients also retry when expired tokens surface as HTTP 400. | **Prerequisite DONE 2026-08-06: the ITM.Web/ITM.API prod release (runs 7430/7431) is deployed and verified** (sproc auto-applied via sp-delta, new DLLs live, OAuth mcp_ session validated on a v1-backed prod route). v1.0.18 prod deploy is now unblocked; only the ITM.MCP develop->main deploy remains. |
